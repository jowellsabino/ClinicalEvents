...
FROM
    v500.dd_session dds
    inner join v500.dd_contribution ddc on ddc.dd_contribution_id = dds.parent_entity_id
    inner join v500.dd_session_data ddd on ddd.dd_session_id = dds.dd_session_id
    inner join v500.prsnl prs on prs.person_id = dds.session_user_id
    inner join v500.clinical_event ce on ce.parent_event_id = ddc.mdoc_event_id
    ...
    inner join v500.code_value note_type on note_type.code_value = ce.event_cd
           and note_type.code_set = 72
    inner join v500.code_value result_status on result_status.code_value = ce.result_status_cd
           and result_status.code_set = 8
    inner join v500.code_value event_class on event_class.code_value = ce.event_class_cd
           and event_class.cdf_meaning = 'MDOC'
           and event_class.code_set = 53
    inner join v500.prsnl pra on pra.person_id = ce.performed_prsnl_id
    inner join v500.person p on p.person_id = ce.person_id
          AND p.person_id = 3618574 /* [STM.STATTicket.com/DataBase/Oracle/Query#Millennium Patient Identifiers#PATIENTID.STM] */ 
    inner join v500.person_alias pa on pa.person_id = p.person_id
           and pa.active_ind = 1
    ...
    inner join v500.code_value alias_type  on alias_type.code_value = pa.person_alias_type_cd
           and alias_type.cdf_meaning ='MRN'
           and alias_type.code_set = 4
    inner join v500.code_value alias_pool on alias_pool.code_value = pa.alias_pool_cd
           and alias_pool.display = 'CHB_MRN'
           and alias_pool.code_set = 263
    inner join v500.encntr_alias ea on ea.encntr_id = ce.encntr_id
	       ...
	       and ea.active_ind = 1
    inner join v500.code_value encntr_alias_type on encntr_alias_type.code_value = ea.encntr_alias_type_cd
...
where dds.parent_entity_name = 'DD_CONTRIBUTION'
  ...
  and dds.session_user_id =  8531805 /* [STM.STATTicket.com/DataBase/Oracle/Query#Millennium Personnel Identifiers#PERSONNELID.STM] */
